...
function resetWitnessUI(options = {}) {
  const { hardHide = false } = options;
  ...
  if (witnessScreen) {
    ...
    witnessScreen.style.pointerEvents = hardHide ? 'none' : '';
    if (hardHide) witnessScreen.style.opacity = '0';
  }
  if (grid) {
    grid.innerHTML = '';
    ...
    grid.style.pointerEvents = hardHide ? 'none' : '';
    grid.style.display = hardHide ? 'none' : '';
  }
  if (decArrivalLine) {
    decArrivalLine.textContent = '';
    decArrivalLine.style.transition = 'none';
    decArrivalLine.style.opacity = '0';
    decArrivalLine.style.transform = 'translateY(8px)';
    decArrivalLine.style.pointerEvents = 'none';
    decArrivalLine.style.display = hardHide ? 'none' : '';
  }
  if (decArrivalSub) {
    decArrivalSub.textContent = '';
    decArrivalSub.style.transition = 'none';
    decArrivalSub.style.opacity = '0';
    decArrivalSub.style.transform = 'translateY(8px)';
    decArrivalSub.style.pointerEvents = 'none';
    decArrivalSub.style.display = hardHide ? 'none' : '';
  }
  if (decTapHint) {
    decTapHint.textContent = '';
    decTapHint.style.opacity = '0';
    decTapHint.style.pointerEvents = 'none';
  }
}
...
function goHome() {
  ...
  clearGhosts();
  // Clear witness/decohere state hard before rendering home
  resetWitnessUI({ hardHide: true });
  ...
  const oldReturnWhisper = document.getElementById('home-return-whisper');
  if (oldReturnWhisper) oldReturnWhisper.remove();
  if (collapseCount > 0) {
    const whisperEl = document.createElement('div');
    whisperEl.id = 'home-return-whisper';
    ...
    activityTimeout(homeToken, () => { if (document.body.contains(whisperEl)) whisperEl.style.opacity = '1'; }, 2800);
    activityTimeout(homeToken, () => { if (document.body.contains(whisperEl)) whisperEl.style.opacity = '0'; }, 7000);
    activityTimeout(homeToken, () => { if (document.body.contains(whisperEl)) whisperEl.remove(); }, 9000);
  }
  activityTimeout(homeToken, () => {
